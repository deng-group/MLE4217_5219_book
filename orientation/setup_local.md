...
## HPC Access Setup (Quick Start)
For detailed HPC information, you'll learn more in Week 3. For now, let's ensure you can access NUS HPC clusters:

### Prerequisites
- NUS VPN must be installed and connected when off-campus
- Valid NUSNetID with HPC access enabled
- SSH client (Terminal on Mac/Linux, PuTTY on Windows)

### Test Your HPC Access
**Step 1: Connect to VPN**
```bash
# Connect to NUS VPN (off-campus only)
# Follow NUS VPN client instructions to connect
```

**Step 2: Test SSH Connection**
```bash
# Replace YOUR_NUSNETID with your actual NUSNetID
ssh YOUR_NUSNETID@aspire1.nus.edu.sg
```

**Expected result:**
- You should see a login prompt
- Enter your NUS password
- You'll see a welcome message or shell prompt

```{admonition} Success?
:class: tip
If you successfully connected to ASPIRE1, your HPC access is working! If you encountered errors, note them for Week 3 troubleshooting.
```

### Basic HPC Commands (Preview)
Once connected, try these commands:
```bash
# Check who you are logged in as
whoami

# Check available CPU cores on the node you're connected to
lscpu | grep "^CPU(s)" | head -n 1

# List available software modules
module avail

# Check your current jobs in the queue
squeue -u $USER
```

```{admonition} What's Next?
:class: tip
In Week 3, we'll cover Git workflow and more HPC topics in detail. For now, just verify your access works and note any issues to discuss in class.
```
...
# Check available CPU cores on node you're connected to
lscpu | grep "^CPU(s)" | head -n 1
